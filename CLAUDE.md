...
支持 DeepSeek、Kimi、硅基流动、阶跃星辰、智谱、MiniMax、火山引擎、讯飞（见 `PROVIDER_TEMPLATES`）。每家是一份「模板」：`endpoint` + `method` + `authType` + `responseMapping`。
...
- 所有供应商：`Authorization: Bearer <apiKey>`。
...
- **无代理**（静态托管如 GitHub Pages）：浏览器直连各供应商 endpoint。此时**只能用支持 CORS 的供应商**（DeepSeek/Kimi/硅基/阶跃/智谱等），MiniMax/火山/讯飞会被浏览器 CORS 拦截。直连模式下浏览器禁止设置 `Accept-Language`，但解析字段与语言无关，智谱 CP 仍可用。
...
- **API KEY 绝不入库**：仓库里的模板 `apiKey` 恒为空字符串，真实 key 只存在用户设备（手机 storage / 浏览器 localStorage），因此仓库可安全公开。
